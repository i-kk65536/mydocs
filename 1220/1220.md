...
3. 自分の開発したパートの確認方法と結果
    - 確認方法
        ...
        2. 
            ...
            2. `docker compose exec mysql mysql -u appuser -p'APPUSERPW_,aCh9h[a[V' appdb -e'select * from locdata'` を実行し、データベースにアクセスし、テストデータが挿入されていれば正常動作
        3. 
            1. `docker compose exec apiserver python3 client_test_mapview.py` を実行し、`'requredata'`メッセージを送信し、サーバーから`'geoloc'`メッセージを受け取る
            2. コマンドラインに表示されるデータが、緯度、経度、加速度絶対値のみでありデータベース内の値と一致すれば正常動作
